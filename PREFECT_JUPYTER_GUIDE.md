...
| En los logs aparece `Error encountered when computing cache key - result will not be persisted` (`HashError`/`cannot pickle ...`), pero la task igual termina en `Finished in state Completed()` | Ruido inofensivo, no es una falla: Prefect intenta cachear el resultado de la task hasheando sus argumentos, y no puede hacerlo con objetos como una conexión `psycopg2`, un cliente `boto3`, o a veces un `DataFrame` | No requiere acción — la task corrió bien igual. Para silenciarlo, agrega `cache_policy=NO_CACHE` al `@task(...)` de cualquier task que reciba una conexión/cliente/DataFrame como argumento (ya aplicado en `common_tasks.py` y `analisis_chat_th.py`) |
