...
Prefect (`prefect` server, each `prefect-worker-*`, `jupyterhub`) mounts three separate flow locations, each for a different purpose:

- **`/flows/org`** (also `/srv/flows/org` in JupyterHub) → bind-mounted from `./flows` (this git repo). Organizational/template flows, e.g. `flows/analisis_chat_th.py`. Edited via git, not from inside a container.
- **`/flows/shared`** (also `/srv/flows/shared` in JupyterHub) → bind-mounted from `/data/datascience/flows`. Reusable tasks/utilities imported by multiple users' flows (e.g. `common_tasks.py`).
- **`/flows/users`** (also `/home` in JupyterHub) → bind-mounted from `/data/datascience/notebooks`; each user's per-user flow files live next to their notebooks, e.g. `/data/datascience/notebooks/{username}/{project}/prefect_flow.py`.

JupyterHub mounts each of these host paths **twice**, at both names (`/srv/flows/org` and `/flows/org`; `/srv/flows/shared` and `/flows/shared`; `/home` and `/flows/users`) — same files, two doors. This isn't cosmetic: `prefect deploy` with local storage records the working directory you ran it from, and that recorded path has to exist inside the `prefect-worker-*` container that later executes the flow, or the run fails at execution time with a file-not-found error even though the deployment was created successfully. The workers only know the `/flows/...` names, not `/srv/flows/...` or `/home` — so **always `cd` into the `/flows/...` path before running `prefect deploy`** (develop/browse wherever is convenient, e.g. `/home/{username}/...`, but deploy from `/flows/users/{username}/...`).
...
3. **Deploy from a JupyterHub terminal**, standing in the `/flows/...` mirror path (`cd /flows/users/{username}/{project} && prefect deploy my_flow.py:my_flow --name "..." --pool <pool>`) — not from `docker exec -it ds_prefect`. The `prefect` service image is a bare server image without `pandas`/`boto3`/`psycopg2`/`torch`/etc., and it has no `PREFECT_API_URL` set, so a CLI `prefect deploy` run there either fails to import the flow or silently registers against an ephemeral local API instead of the real server. JupyterHub already has both the flow dependencies (see `jupyterhub/Dockerfile`) and `PREFECT_API_URL=http://prefect:4200/api` wired via `c.Spawner.environment`. If `prefect deploy` prompts about pulling flow code from remote storage, answer `n` — the code is already local to the workers via the shared mounts, and the image has no `git` binary for the remote-storage path anyway.
...
- Organizational/shared/user flow code is not separately "owned" by Prefect: `/flows/org` comes from the git repo (`./flows`), `/flows/shared` from `/data/datascience/flows`, `/flows/users` from `/data/datascience/notebooks` (see "Prefect flows" section above). The `prefect-worker-*` services are stateless — none of them has a dedicated persistence directory, they only read these flow mounts.
...
- If you remove or rename a service in `docker-compose.yml`, its old container keeps running — plain `docker compose up -d` only creates/updates services still defined in the file, it does not stop containers for services that disappeared. Use `docker compose up -d --remove-orphans` any time a service was removed/renamed, so the leftover container gets cleaned up instead of quietly running (and, for `prefect-worker-*`, quietly still polling its old pool) forever.
- If you change a Dockerfile, rebuild only the affected service with `docker compose build service_name`.
- Never run `prefect` CLI commands (e.g. `prefect work-pool ls`, `prefect work-pool resume ...`) via `docker exec -it ds_prefect ...` — that container has no `PREFECT_API_URL` set, so each invocation spins up its own temporary ephemeral API server instead of talking to the one already running (you'll see `Starting/Stopping temporary server on http://127.0.0.1:...` in the output). It happens to read/write the same Postgres backend so data isn't lost, but it's wasted overhead and not the intended path — run these from a JupyterHub terminal instead, which already has `PREFECT_API_URL=http://prefect:4200/api` wired.
